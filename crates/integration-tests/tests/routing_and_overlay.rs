...
fn router_encrypt_decrypt_full_simulation() {
    ...
    let mut router_a = Router::new(table, overlay_a);
...
fn router_drops_non_overlay_packets() {
    ...
    let mut router = Router::new(table, overlay_a);
    router.add_transport(id_b, t_a);
...
fn allocation_table_many_peers_lifecycle() {
    ...
    for id in &ids[10..20] {
        assert!(table.lookup_by_peer(id).is_some());
    }
}
